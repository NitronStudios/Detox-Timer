...
export default function Navbar({ tab, setTab, theme, cycleTheme, showDash, setShowDash, showSettings, setShowSettings, showManualLog, setShowManualLog, showLeaderboard, setShowLeaderboard, showChat, setShowChat }) {
  ...
  const ThemeIcon = theme === 'light' ? Sun : theme === 'dark' ? Moon : Clock;
    ...
    <>
      ...
      <div className={`side-menu ${isMobileMenuOpen ? 'open' : ''}`}>
        ...
        <div className="side-menu-links">
          ...
          <button className={showChat ? 'active' : ''} onClick={() => handleMenuClick(() => { setShowChat(true); setShowDash(false); setShowSettings(false); setShowLeaderboard(false); setShowManualLog(false); })}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg>
            <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
              Friends & Chat
              {pendingRequests > 0 && <span style={{ background: '#EF4444', color: '#fff', borderRadius: '50%', padding: '2px 8px', fontSize: '0.7rem', fontWeight: 'bold' }}>{pendingRequests}</span>}
            </span>
          </button>
